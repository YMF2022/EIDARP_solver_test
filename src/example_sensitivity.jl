...
const initE             = 0.3 # 100% of initial battery level
...
# Start the main loop
function analyze_detour(num_cus::Int64, NETWORK::String, vns_params::VNSParameter, detour_factor::Float64)
    ...
    result_folder = "results/metaheuristics/isE/" * folder * "_detour$detour_factor"
    # result_folder = "results/metaheuristics/" * folder * "/$iteration"
    if !isdir(result_folder) mkdir(result_folder) end
    eidarp, ts_network, ts_network_noshift = preprocess(data_folder, NETWORK, set_dummy_recharge = true, max_recharge = max_recharge, set_detour = detour_factor);
    ...
    s, cost, results, iter, t_solve, count_neb_change, res_localserach = VNS(s₀, vns_params, t_start, degree_destroy, c̄, is_rejection = true);
    ...
    recharge_conflict_check_final(s)
    return 
end

detour_list = [1.6, 1.7, 1.8, 1.9, 2.0, 2.1, 2.2, 2.3, 2.4, 2.5]
# detour_list = [1.6]
for detour_factor in detour_list
    analyze_detour(100, "cross", vns_params, detour_factor)
end
analyze_detour(100, "cross", vns_params, 2.5)
